...
#include "ndn-sync-real-locality.hpp"
...
NS_LOG_COMPONENT_DEFINE("ndn.RealGraphLocality");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED(RealGraphLocality);

// register NS-3 type
TypeId
RealGraphLocality::GetTypeId()
{
  static TypeId tid = TypeId("ns3::ndn::RealGraphLocality").SetGroupName("Ndn").SetParent<App>().AddConstructor<RealGraphLocality>()
  .AddAttribute("Prefix", "Name of the Interest", StringValue("/"),
                MakeNameAccessor(&RealGraphLocality::m_prefix), MakeNameChecker())
  .AddAttribute("Function", "Name of Function", StringValue("/"),
                MakeNameAccessor(&RealGraphLocality::m_funcName), MakeNameChecker())
  .AddAttribute("Hint", "Forwarding hint of node", StringValue("/"),
                MakeNameAccessor(&RealGraphLocality::m_hint), MakeNameChecker())
  .AddAttribute("NumNodes", "Number of nodes", UintegerValue(30),
                MakeUintegerAccessor(&RealGraphLocality::m_nodes_num), MakeUintegerChecker<uint32_t>())
  .AddAttribute("NumNode", "Number of this node", UintegerValue(0),
                MakeUintegerAccessor(&RealGraphLocality::m_node_num), MakeUintegerChecker<uint32_t>());
  return tid;
}

RealGraphLocality::RealGraphLocality()
  : m_graph(root)
...
void
RealGraphLocality::StartApplication()
{
  // initialize ndn::App
  ...
  auto it = m_computegraph.findUpdate(m_val.Get());
  std::cout << "Scheduling Interest after: " << int(it.start) + 1 << "seconds\n";
  Simulator::Schedule(Seconds(int(it.start) + 1), &RealGraphLocality::DispatchRequest, this, m_val, false);
  Simulator::Schedule(Seconds(0.2), &RealGraphLocality::SendInitialInterest, this);

  //uint32_t updateTime = m_updateGen->GetValue(0, 2000);
  // Simulator::Schedule(MilliSeconds(updateTime), &RealGraphLocality::GenerateUpdate, this);

  // Util Update Period
  m_period = ns3::Seconds(1);
  Simulator::Schedule(m_period, &RealGraphLocality::sendUtilInterest, this);
  m_nodeToSend = "";
  m_cores = 4;
  m_util = 0;
  m_hopLimit = 2;
...
void
RealGraphLocality::StopApplication()
{
  // cleanup ndn::App
...
void
RealGraphLocality::OnInterest(std::shared_ptr<const ndn::Interest> interest)
{
  ...
  // else if (interest->getName().get(-1).toUri() == "initial") {
  //   uint32_t hopCount = 0;
  ...
  //
  //   NS_LOG_DEBUG("Sending back response to initial Interest " << data->getName());
  //   return;
  // }
    ...
    if (std::stoi(interest->getName().get(2).toUri()) > 1000) {
    ...
    if (thunkOfMaxInput != "") {
      ...
      delay = hops * 2 * 0.1 + std::stoi(interest->getName().get(3).toUri());
...
void
RealGraphLocality::OnData(std::shared_ptr<const ndn::Data> data)
{
  if (data->getName().get(-1).toUri() == "NACK") {
    ...
    //   Simulator::Schedule(Seconds(1), &RealGraphLocality::DispatchRequest, this, counter, true);
    //   return;
    // }
    else {
      if (m_nodes.size() == 1) {
        Simulator::Schedule(Seconds(5), &RealGraphLocality::DispatchRequest, this, counter, true);
        return;
      }
    ...
    Simulator::Schedule(Seconds(1), &RealGraphLocality::DispatchRequest, this, counter, false);
    return;
  }
  ...
  // }
    ...
    Simulator::Schedule(Seconds(std::stod(data->getName().get(-2).toUri())), &RealGraphLocality::SendThunkInterest, this, data);
    ...
    if (m_val.Get() < m_computegraph.items.size())
      Simulator::Schedule(Seconds(1), &RealGraphLocality::DispatchRequest, this, m_val.Get(), false);
    return;
  }
...
void
RealGraphLocality::GenerateUpdate(int val)
{
  NS_LOG_DEBUG("Generating update");

  //auto it = m_computegraph.items.begin();
  auto info = m_computegraph.findUpdate(val);
  //std::advance(it, val);
  //auto info = *it;
...
RealGraphLocality::findVertex(std::string vertexId)
{
  boost::graph_traits<Graph>::vertex_iterator vi, vi_end;
...
void
RealGraphLocality::handleUpdate(std::shared_ptr<const ndn::Interest> interest)
{
...
void
RealGraphLocality::requestMissingUpdate(std::string updateHash)
{
...
void
RealGraphLocality::sendBackUpdate(std::shared_ptr<const ndn::Interest> interest)
{
...
void
RealGraphLocality::DispatchRequest(ns3::IntegerValue val, bool pickNodeToSend)
{
  if (is_node_stopped) {
    return;
  }
  ...
  objectInfo info = m_computegraph.findUpdate(val.Get());
...
void
RealGraphLocality::executeLocally(ns3::IntegerValue val)
{
  if (m_util >= m_cores) {
    Simulator::Schedule(Seconds(1), &RealGraphLocality::executeLocally, this, val);
    return;
  }
  Simulator::Schedule(Seconds(1), &RealGraphLocality::decrementUtil, this);
  m_util++;
  //auto it = m_computegraph.items.begin();
  // std::advance(it, val.Get());
  objectInfo info = m_computegraph.findUpdate(val.Get());
...
void
RealGraphLocality::decrementUtil()
{
  m_util--;
}

void
RealGraphLocality::sendBackThunk(Name interestName, double delay)
{
  Name thunkName = Name(m_hint.toUri());
...
void
RealGraphLocality::SendThunkInterest(std::shared_ptr<const ndn::Data> data)
{
  Block thunkBlock = data->getContent();
  thunkBlock.parse();
  Name thunkName;
...
void
RealGraphLocality::sendUtilInterest()
{
  // util prefix
  Name name = Name("/util");
  // hint of node
...
void
RealGraphLocality::handleUtilInterest(std::shared_ptr<const ndn::Interest> interest)
{
...
void
RealGraphLocality::SendInitialInterest() {
...
void
RealGraphLocality::sendNackData(Name name) {
...
void
RealGraphLocality::sendRetryData(Name name) {
  Name intName = Name(name.toUri());
