...
#include "ndn-sync-forwarding-util.hpp"
...
NS_LOG_COMPONENT_DEFINE("ndn.SyncForwardingUtil");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED(SyncForwardingUtil);

// register NS-3 type
TypeId
SyncForwardingUtil::GetTypeId()
{
  static TypeId tid = TypeId("ns3::ndn::SyncForwardingUtil").SetGroupName("Ndn").SetParent<App>().AddConstructor<SyncForwardingUtil>()
  .AddAttribute("Prefix", "Name of the Interest", StringValue("/"),
                MakeNameAccessor(&SyncForwardingUtil::m_prefix), MakeNameChecker())
  .AddAttribute("Function", "Name of Function", StringValue("/"),
                MakeNameAccessor(&SyncForwardingUtil::m_funcName), MakeNameChecker())
  .AddAttribute("Hint", "Forwarding hint of node", StringValue("/"),
                MakeNameAccessor(&SyncForwardingUtil::m_hint), MakeNameChecker());
  return tid;
}

SyncForwardingUtil::SyncForwardingUtil()
  : m_graph(root)
...
void
SyncForwardingUtil::StartApplication()
{
  // initialize ndn::App
  ...
  Simulator::Schedule(MilliSeconds(updateTime), &SyncForwardingUtil::GenerateUpdate, this);

  // Util Update Period
  m_period = ns3::Seconds(5);
  Simulator::Schedule(m_period, &SyncForwardingUtil::sendUtilInterest, this);
...
void
SyncForwardingUtil::StopApplication()
{
  // cleanup ndn::App
...
void
SyncForwardingUtil::OnInterest(std::shared_ptr<const ndn::Interest> interest)
{
...
void
SyncForwardingUtil::OnData(std::shared_ptr<const ndn::Data> data)
{
  if (data->getName().get(2).toUri() == "update") {
    ...
    Simulator::Schedule(MilliSeconds(0), &SyncForwardingUtil::SendThunkInterest, this, data);
    return;
  }
...
void
SyncForwardingUtil::GenerateUpdate()
{
...
SyncForwardingUtil::findVertex(std::string vertexId)
{
  boost::graph_traits<Graph>::vertex_iterator vi, vi_end;
...
void
SyncForwardingUtil::handleUpdate(std::shared_ptr<const ndn::Interest> interest)
{
...
void
SyncForwardingUtil::requestMissingUpdate(std::string updateHash)
{
...
void
SyncForwardingUtil::sendBackUpdate(std::shared_ptr<const ndn::Interest> interest)
{
...
void
SyncForwardingUtil::DispatchRequest(std::string hash, Name function, uint32_t dataSize)
{
...
void
SyncForwardingUtil::sendBackThunk(Name interestName)
{
...
void
SyncForwardingUtil::SendThunkInterest(std::shared_ptr<const ndn::Data> data)
{
...
void
SyncForwardingUtil::sendUtilInterest()
{
  // util prefix
  Name name = Name("/util");
  // hint of node
...
void
SyncForwardingUtil::handleUtilInterest(std::shared_ptr<const ndn::Interest> interest)
{
