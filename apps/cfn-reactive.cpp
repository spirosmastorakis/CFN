...
NS_LOG_COMPONENT_DEFINE("ndn.CFNReactive");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED(CFNReactive);

// register NS-3 type
TypeId
CFNReactive::GetTypeId()
{
  static TypeId tid = TypeId("ns3::ndn::CFNReactive").SetGroupName("Ndn").SetParent<App>().AddConstructor<CFNReactive>()
  .AddAttribute("Prefix", "Name of the Interest", StringValue("/"),
                MakeNameAccessor(&CFNReactive::m_prefix), MakeNameChecker())
  .AddAttribute("Function", "Name of Function", StringValue("/"),
                MakeNameAccessor(&CFNReactive::m_funcName), MakeNameChecker())
  .AddAttribute("Hint", "Forwarding hint of node", StringValue("/"),
                MakeNameAccessor(&CFNReactive::m_hint), MakeNameChecker())
  .AddAttribute("NumNodes", "Number of nodes", UintegerValue(30),
                MakeUintegerAccessor(&CFNReactive::m_nodes_num), MakeUintegerChecker<uint32_t>())
  .AddAttribute("EnableCallerRelation", "Caller relation enabled", UintegerValue(1),
                MakeUintegerAccessor(&CFNReactive::m_caller_enabled), MakeUintegerChecker<uint32_t>())
  .AddAttribute("ParamK", "Value of param k", UintegerValue(10),
                MakeUintegerAccessor(&CFNReactive::k), MakeUintegerChecker<uint32_t>())
  .AddAttribute("NumNode", "Number of this node", UintegerValue(0),
                MakeUintegerAccessor(&CFNReactive::m_node_num), MakeUintegerChecker<uint32_t>());
  return tid;
}

CFNReactive::CFNReactive()
  : m_graph(root)
...
CFNReactive::StartApplication()
{
  ...
  if (m_caller_enabled == 1) {
    ...
    if (it.caller == "") {
      ...
      //Simulator::Schedule(Seconds(int(it.start) + 1), &CFNReactive::DispatchRequest, this, m_val, false);
      Simulator::Schedule(Seconds(1), &CFNReactive::DispatchRequest, this, m_val, false, false);
    }
  }
  else {
    NS_LOG_DEBUG("Global counter: " << m_val.Get());
  ...
  Simulator::Schedule(Seconds(0.2), &CFNReactive::SendInitialInterest, this);

  //uint32_t updateTime = m_updateGen->GetValue(0, 2000);
  // Simulator::Schedule(MilliSeconds(updateTime), &CFNReactive::GenerateUpdate, this);

  // Util Update Period
  m_period = ns3::Seconds(1);
  Simulator::Schedule(m_period, &CFNReactive::sendUtilInterest, this);
  m_nodeToSend = "";
  m_cores = 4;
  m_util = 0;
  m_hopLimit = 2;
...
void
CFNReactive::OnInterest(std::shared_ptr<const ndn::Interest> interest)
{
  ...
  if (interest->getName().get(-2).toUri() == "initial") {
    ...
    m_util++;
    Simulator::Schedule(Seconds(std::stoi(interest->getName().get(3).toUri())), &CFNReactive::decrementUtil, this);
...
void
CFNReactive::OnData(std::shared_ptr<const ndn::Data> data)
{
  ...
  if (data->getName().get(-1).toUri() == "NACK") {
    ...
    //   Simulator::Schedule(Seconds(1), &CFNReactive::DispatchRequest, this, counter, true);
    //   return;
    // }
    else {
      if (m_nodes.size() == 1) {
        Simulator::Schedule(Seconds(5), &CFNReactive::DispatchRequest, this, counter, true, false);
        return;
      }
      ...
      for (auto it2 = m_util_vector.end() - 1; it2 != m_util_vector.begin() - 1; it2--) {
        if (std::get<1>(*it2) < 1.0) {
          ...
          Simulator::Schedule(Seconds(1), &CFNReactive::DispatchRequest, this, counter, true, false);
          return;
        }
      }
      Simulator::Schedule(Seconds(5), &CFNReactive::DispatchRequest, this, counter, true, false);
  ...
  // }
    ...
    Simulator::Schedule(Seconds(std::stod(data->getName().get(-2).toUri())), &CFNReactive::SendThunkInterest, this, data);
    ...
    if (m_val.Get() < m_computegraph.items.size()) {
      if (m_caller_enabled == 0) {
        ...
        Simulator::Schedule(Seconds(0.1), &CFNReactive::DispatchRequest, this, m_val.Get(), false, false);
      }
      else {
...
void
CFNReactive::GenerateUpdate(int val)
{
...
CFNReactive::findVertex(std::string vertexId)
{
  boost::graph_traits<Graph>::vertex_iterator vi, vi_end;
...
void
CFNReactive::handleUpdate(std::shared_ptr<const ndn::Interest> interest)
{
...
void
CFNReactive::requestMissingUpdate(std::string updateHash)
{
...
void
CFNReactive::sendBackUpdate(std::shared_ptr<const ndn::Interest> interest)
{
...
void
CFNReactive::DispatchRequest(ns3::IntegerValue val, bool pickNodeToSend, bool re_execution)
{
  if (is_node_stopped) {
    return;
  }
  ...
  for (auto it = m_deadNodes.begin(); it != m_deadNodes.end(); it++) {
    if (it->isPrefixOf(hintOfMaxData)) {
      hintOfMaxData = m_nodeToSend.toUri();
    }
  }
  ...
  auto event = Simulator::Schedule(Seconds(2.1), &CFNReactive::NodeFailed, this, intName);
...
void
CFNReactive::executeLocally(ns3::IntegerValue val)
{
  if (m_util >= m_cores) {
    m_nodeToSend = (m_nodes.end() - 1)->first.toUri();
    Simulator::Schedule(Seconds(1), &CFNReactive::DispatchRequest, this, m_val.Get(), true, false);
    return;
  }
  Simulator::Schedule(Seconds(1), &CFNReactive::decrementUtil, this);
  m_util++;
  //auto it = m_computegraph.items.begin();
  // std::advance(it, val.Get());
  objectInfo info = m_computegraph.findUpdate2(val.Get());
...
void
CFNReactive::decrementUtil()
{
  m_util--;
}

void
CFNReactive::sendBackThunk(Name interestName, double delay)
{
  m_numTasks++;
  Name thunkName = Name(m_hint.toUri());
...
void
CFNReactive::SendThunkInterest(std::shared_ptr<const ndn::Data> data)
{
  Block thunkBlock = data->getContent();
  thunkBlock.parse();
  Name thunkName;
  ...
  auto event = Simulator::Schedule(Seconds(1.1), &CFNReactive::NodeFailed, this, thunkName);
...
void
CFNReactive::handleUtilInterest(std::shared_ptr<const ndn::Interest> interest)
{
...
void
CFNReactive::SendInitialInterest() {
...
void
CFNReactive::sendNackData(Name name) {
...
void
CFNReactive::sendRetryData(Name name) {
  Name intName = Name(name.toUri());
...
void
CFNReactive::RequestInputs(Name taskName)
{
  int counter = 0;
...
void
CFNReactive::SendInputInterest(Name inputName)
{
  Ptr<UniformRandomVariable> rand = CreateObject<UniformRandomVariable>();
  ...
  auto event = Simulator::Schedule(Seconds(1.1), &CFNReactive::NodeFailed, this, name);
...
void
CFNReactive::NodeFailed(Name name)
{
  if (is_node_stopped) {
    return;
  }
