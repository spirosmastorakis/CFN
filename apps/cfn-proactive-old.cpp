...
NS_LOG_COMPONENT_DEFINE("ndn.CFNProactiveOld");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED(CFNProactiveOld);

// register NS-3 type
TypeId
CFNProactiveOld::GetTypeId()
{
  static TypeId tid = TypeId("ns3::ndn::CFNProactiveOld").SetGroupName("Ndn").SetParent<App>().AddConstructor<CFNProactiveOld>()
  .AddAttribute("Prefix", "Name of the Interest", StringValue("/"),
                MakeNameAccessor(&CFNProactiveOld::m_prefix), MakeNameChecker())
  .AddAttribute("Function", "Name of Function", StringValue("/"),
                MakeNameAccessor(&CFNProactiveOld::m_funcName), MakeNameChecker())
  .AddAttribute("Hint", "Forwarding hint of node", StringValue("/"),
                MakeNameAccessor(&CFNProactiveOld::m_hint), MakeNameChecker())
  .AddAttribute("NumNodes", "Number of nodes", UintegerValue(30),
                MakeUintegerAccessor(&CFNProactiveOld::m_nodes_num), MakeUintegerChecker<uint32_t>())
  .AddAttribute("EnableCallerRelation", "Caller relation enabled", UintegerValue(1),
                MakeUintegerAccessor(&CFNProactiveOld::m_caller_enabled), MakeUintegerChecker<uint32_t>())
  .AddAttribute("ParamK", "Value of param k", UintegerValue(10),
                MakeUintegerAccessor(&CFNProactiveOld::k), MakeUintegerChecker<uint32_t>())
  .AddAttribute("NumNode", "Number of this node", UintegerValue(0),
                MakeUintegerAccessor(&CFNProactiveOld::m_node_num), MakeUintegerChecker<uint32_t>());
  return tid;
}

CFNProactiveOld::CFNProactiveOld()
  : m_graph(root)
...
void
CFNProactiveOld::StartApplication()
{
  // initialize ndn::App
  ...
  if (m_caller_enabled == 1) {
    ...
    if (it.caller == "") {
      ...
      //Simulator::Schedule(Seconds(int(it.start) + 1), &CFNProactiveOld::DispatchRequest, this, m_val, false);
      Simulator::Schedule(Seconds(1), &CFNProactiveOld::DispatchRequest, this, m_val, false, false);
    }
  }
  else {
    NS_LOG_DEBUG("Global counter: " << m_val.Get());
  ...
  Simulator::Schedule(Seconds(0.2), &CFNProactiveOld::SendInitialInterest, this);

  //uint32_t updateTime = m_updateGen->GetValue(0, 2000);
  // Simulator::Schedule(MilliSeconds(updateTime), &CFNProactiveOld::GenerateUpdate, this);

  // Util Update Period
  m_period = ns3::Seconds(1);
  Simulator::Schedule(m_period, &CFNProactiveOld::sendUtilInterest, this);
  m_nodeToSend = "";
  m_cores = 4;
  m_util = 0;
  m_hopLimit = 2;
...
void
CFNProactiveOld::StopApplication()
{
  // cleanup ndn::App
...
void
CFNProactiveOld::OnInterest(std::shared_ptr<const ndn::Interest> interest)
{
  ...
  if (interest->getName().get(-2).toUri() == "initial") {
    ...
    m_util++;
    Simulator::Schedule(Seconds(std::stoi(interest->getName().get(3).toUri())), &CFNProactiveOld::decrementUtil, this);
...
void
CFNProactiveOld::OnData(std::shared_ptr<const ndn::Data> data)
{
  ...
  if (data->getName().get(-1).toUri() == "NACK") {
    ...
    //   Simulator::Schedule(Seconds(1), &CFNProactiveOld::DispatchRequest, this, counter, true);
    //   return;
    // }
    else {
      if (m_nodes.size() == 1) {
        Simulator::Schedule(Seconds(5), &CFNProactiveOld::DispatchRequest, this, counter, true, false);
        return;
      }
      ...
      for (auto it2 = m_util_vector.end() - 1; it2 != m_util_vector.begin() - 1; it2--) {
        if (std::get<1>(*it2) < 1.0) {
          ...
          Simulator::Schedule(Seconds(1), &CFNProactiveOld::DispatchRequest, this, counter, true, false);
          return;
        }
      }
      Simulator::Schedule(Seconds(5), &CFNProactiveOld::DispatchRequest, this, counter, true, false);
  ...
  // }
    ...
    Simulator::Schedule(Seconds(std::stod(data->getName().get(-2).toUri())), &CFNProactiveOld::SendThunkInterest, this, data);
    ...
    if (m_val.Get() < m_computegraph.items.size()) {
      if (m_caller_enabled == 0) {
        ...
        Simulator::Schedule(Seconds(0.1), &CFNProactiveOld::DispatchRequest, this, m_val.Get(), false, false);
      }
      else {
...
void
CFNProactiveOld::GenerateUpdate(int val)
{
...
CFNProactiveOld::findVertex(std::string vertexId)
{
  boost::graph_traits<Graph>::vertex_iterator vi, vi_end;
...
void
CFNProactiveOld::handleUpdate(std::shared_ptr<const ndn::Interest> interest)
{
...
void
CFNProactiveOld::requestMissingUpdate(std::string updateHash)
{
...
void
CFNProactiveOld::sendBackUpdate(std::shared_ptr<const ndn::Interest> interest)
{
...
void
CFNProactiveOld::DispatchRequest(ns3::IntegerValue val, bool pickNodeToSend, bool re_execution)
{
  if (is_node_stopped) {
    return;
  }
  ...
  auto event = Simulator::Schedule(Seconds(2.1), &CFNProactiveOld::NodeFailed, this, intName);
...
void
CFNProactiveOld::executeLocally(ns3::IntegerValue val)
{
  if (m_util >= m_cores) {
    Simulator::Schedule(Seconds(1), &CFNProactiveOld::executeLocally, this, val);
    return;
  }
  Simulator::Schedule(Seconds(1), &CFNProactiveOld::decrementUtil, this);
  m_util++;
  //auto it = m_computegraph.items.begin();
  // std::advance(it, val.Get());
  objectInfo info = m_computegraph.findUpdate2(val.Get());
  ...
  std::cout << "m_computegraph.items.size(): " << m_computegraph.items.size() << std::endl;
  if (m_val.Get() < m_computegraph.items.size())
    Simulator::Schedule(Seconds(1), &CFNProactiveOld::DispatchRequest, this, m_val.Get(), false, false);
  return;
}

void
CFNProactiveOld::decrementUtil()
{
  m_util--;
}

void
CFNProactiveOld::sendBackThunk(Name interestName, double delay)
{
  Name thunkName = Name(m_hint.toUri());
...
void
CFNProactiveOld::SendThunkInterest(std::shared_ptr<const ndn::Data> data)
{
  Block thunkBlock = data->getContent();
  thunkBlock.parse();
  Name thunkName;
  ...
  auto event = Simulator::Schedule(Seconds(1.1), &CFNProactiveOld::NodeFailed, this, thunkName);
...
void
CFNProactiveOld::sendUtilInterest()
{
  if (is_node_stopped)
    return;
...
void
CFNProactiveOld::handleUtilInterest(std::shared_ptr<const ndn::Interest> interest)
{
  ...
  for (auto i = m_deadNodes.begin(); i != m_deadNodes.end(); i++) {
    //std::cout << "hint: " << hint << std::endl;
    //std::cout << "std::get<0>(*i).toUri(): " << std::get<0>(*i).toUri() << std::endl;
    if (*i == hint) {
      m_deadNodes.erase(i);
...
void
CFNProactiveOld::SendInitialInterest() {
...
void
CFNProactiveOld::sendNackData(Name name) {
...
void
CFNProactiveOld::sendRetryData(Name name) {
  Name intName = Name(name.toUri());
...
void
CFNProactiveOld::RequestInputs(Name taskName)
{
  int counter = 0;
...
void
CFNProactiveOld::SendInputInterest(Name inputName)
{
  Ptr<UniformRandomVariable> rand = CreateObject<UniformRandomVariable>();
  ...
  auto event = Simulator::Schedule(Seconds(1.1), &CFNProactiveOld::NodeFailed, this, name);
...
void
CFNProactiveOld::NodeFailed(Name name)
{
  if (is_node_stopped) {
    return;
  }
  ...
  if (add) {
    ...
    for (auto it = m_computegraph.items.begin(); it != m_computegraph.items.end(); it++) {
        ...
        if (hint.isPrefixOf(Name(it->thunk))) {
          val.Set(counter);
          std::cout << "Re-executing request: " << info.name << std::endl;
          DispatchRequest(val, true, true);
        }
        // else if (name.getPrefix(2).toUri() == it->name) {
        //   val.Set(counter);
        //   std::cout << "Re-executing request: " << info.name << std::endl;
        //   DispatchRequest(val, true, true);
        // }
        // else if (hint.isPrefix() {
        //
        // }
        counter++;
      }
  ...
  if (name.get(-1).toUri() == "input") {
    ...
    DispatchRequest(val, true, true);
    //Simulator::Schedule(Seconds(0.5), &CFNReactive::DispatchRequest, this val, true);
  }
  ...
  //     }
  //     struct objectInfo info = m_computegraph.findUpdate2(counter);
  //     std::cout << "name.getSubName(1).toUri(): " << name.getSubName(1).toUri() << std::endl;
  //     std::cout << "it->name: " << it->name << std::endl;
  //     std::cout << "it->thunk: " << it->thunk << std::endl;
  //
  //     if (hint.isPrefixOf(Name(it->thunk))) {
  ...
  //       DispatchRequest(val, true, true);
  //     }
  //     else if (name.getSubName(1).toUri() == it->name) {
  //       val.Set(counter);
  //       std::cout << "Re-executing request: " << info.name << std::endl;
  //       DispatchRequest(val, true, true);
  //     }
